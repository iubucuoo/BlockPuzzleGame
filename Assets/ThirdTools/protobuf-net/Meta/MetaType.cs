...
namespace ProtoBuf.Meta
{
    ...
    public class MetaType : ISerializerProxy
    {
        ...
        private IProtoTypeSerializer BuildSerializer()
        {
            ...
            if (subTypeCount != 0)
            {
                var item = subTypes.GetEnumerator();
                while(item.MoveNext())
                {
                    SubType subType = item.Current as SubType;
#if WINRT
            ...
            if (fieldCount != 0)
            {
                var item = fields.GetEnumerator();
                while(item.MoveNext())
                {
                    ValueMember member = item.Current as ValueMember;
        ...
        private static ProtoMemberAttribute NormalizeProtoMember(TypeModel model, MemberInfo member, AttributeFamily family, bool forced, bool isEnum, BasicList partialMembers, int dataMemberOffset, bool inferByTagName)
        {
            ...
            if (!ignore && !done) // always consider ProtoMember 
            {
                ...
                {
                    var item = partialMembers.GetEnumerator();
                    while(item.MoveNext())
                    {
                        AttributeMap ppma = item.Current as AttributeMap;
                        object tmp;
            ...
            if (!ignore && !done && HasFamily(family, AttributeFamily.DataContractSerialier))
            {
                ...
                if (attrib != null)
                {
                    string seq = "Ord";
                    seq += "er";
                    GetFieldNumber(ref fieldNumber, attrib, seq);
                    GetFieldName(ref name, attrib, "Name");
                    GetFieldBoolean(ref isRequired, attrib, "IsRequired");
        ...
        public ValueMember this[MemberInfo member]
        {
            get
            {
                ...
                while(item.MoveNext())
                {
                    ValueMember x = item.Current as ValueMember;
        ...
        internal bool IsDefined(int fieldNumber)
        {
            ...
            while(item.MoveNext())
            {
                ValueMember field = item.Current as ValueMember;
...
.IsDefined(model.MapType(typeof(FlagsAttribute)), false))
                ...
                {
                    var item = fieldsArr.GetEnumerator();
                    while(item.MoveNext())
                    {
                        ValueMember member = item.Current as ValueMember;
