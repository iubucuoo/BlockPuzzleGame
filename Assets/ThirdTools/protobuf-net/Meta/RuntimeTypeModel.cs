...
namespace ProtoBuf.Meta
{
    ...
    public sealed class RuntimeTypeModel : TypeModel
    {
        ...
        /// <summary>
        /// Global default that
        /// enables/disables automatic tag generation based on the existing name /
        ...
        /// <summary>
        /// Global  that enables or disables the implicit
        ...
        /// <summary>
        /// Global  that determines whether types with a <c>.ToString()</c> and a <c>Parse(string)</c>
        ...
        public override string GetSchema(Type type)
        {
            ...
            if (!isInbuiltType)
            {
                ...
                foreach (MetaType meta in typesForNamespace)
                {
                    ...
                    {
                        if (tmp.IndexOf("System.") == 0) continue;
                        if (package == null)
...
//#endif
        ...
        /// [DataContract]/[DataMember(=n)]
        /// [XmlType]/[XmlElement(=n)]
        ...
        internal void ReleaseLock(int opaqueToken)
        {
            if (opaqueToken != 0)
            {
                ...
                if(opaqueToken != GetContention()) // contention-count changes since we looked!
                {
                    ...
                    if (handler != null)
                    {
                        // not hugely elegant, but this is such a far-corner-case that it doesn't need to be slick - I'll settle for cross-
                        string stackTrace;
                        try
                        {
