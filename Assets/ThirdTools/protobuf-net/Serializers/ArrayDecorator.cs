...
namespace ProtoBuf.Serializers
{
    sealed class ArrayDecorator : ProtoDecoratorBase
    {
        ...
        public override object Read(object value, ProtoReader source, bool isNv = false)
        {
            ...
            { 
                do
                {
                    TypeCode code = Type.GetTypeCode(itemType);
                    var v = Tail.Read(null, source);
                    switch (code)
                    {
                        case TypeCode.Single:v =ValueObject.Value<float>(v);break;
                        case TypeCode.Int16: v = ValueObject.Value<short>(v); break;
                        case TypeCode.Int32:v = ValueObject.Value<int>(v);break;
                    }
                    list.Add(v);
